...
class DDPMWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        if self.resample_strategy == "spaced":
            ...
            if self.spaced_diffusion is None:
                ...
                self.spaced_diffusion = sample_nw.to(x.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            ...
            with torch.no_grad():
                ...
                if self.use_adaptive_conditioning:
                    ...
                    mean_var = variance.mean(dim=[1, 2, 3], keepdim=True) # (B, 512, 1, 1) -> (B, 1, 1, 1)
                    
                    adaptive_weight = mean_var * self.variance_scale
                    # adaptive_weight = torch.abs(adaptive_weight) #
                ################################################
        ...
        eps_pred = self.online_network(
            x, eps, t, low_res=cond, z=z.squeeze() if self.z_cond else None, adaptive_weight=adaptive_weight ##
        )
        ...
        ##############################
        if self.use_adaptive_conditioning:
            scale_reg = (self.variance_scale ** 2) ## if variance_scale is not converged, manipulate the weight of 0.001
            loss = loss + scale_reg
            self.log("variance_scale", self.variance_scale.item(), prog_bar=True)
        #################################
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        if self.eval_mode == "sample":
            ...
            with torch.no_grad():
                ...
                variance = torch.exp(logvar)
                mean_var = variance.mean(dim=[1, 2, 3], keepdim=True)
                adaptive_weight = mean_var * self.variance_scale
                recons = self.vae(z)
            ###############################################
            recons = 2 * recons - 1
            ...
            with torch.no_grad():
                ...
                variance = torch.exp(logvar)
                mean_var = variance.mean(dim=[1, 2, 3], keepdim=True)
                adaptive_weight = mean_var * self.variance_scale if self.use_adaptive_conditioning else None
                # adaptive_weight = torch.abs(adaptive_weight)
        ...
        adaptive_weight = adaptive_weight[:, 0, 0, 0]
        adaptive_weight = torch.mean(adaptive_weight)
    ...
    def configure_optimizers(self):
        ###################################
        params = list(self.online_network.decoder.parameters())
        if self.use_adaptive_conditioning:
            params.append(self.variance_scale)
        #########################################
        # optimizer = torch.optim.Adam(self.online_network.decoder.parameters(), lr=self.lr)
