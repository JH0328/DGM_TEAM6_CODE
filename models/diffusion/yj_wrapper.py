...
class yj_DDPMWrapper(pl.LightningModule):
    def __init__(
        self,
        online_network,
        target_network,
    ...
    def training_step(self, batch, batch_idx):
        ...
        # !^L Add scaling loss strategy for memory saving
        loss_scaler= torch.cuda.amp.GradScaler() # Loss scaling for stability
        #!^L Add annealing scheduler
        def annealing_weight(epoch= self.current_epoch, start=1.0, end=0.0, rate=.998):
...
# VAE_loss
        ...
        recons_loss = mse_loss(self.vae.forward_recons(x), x)
        kl_loss = self.vae.compute_kl(mu, logvar)
        vae_loss = recons_loss + self.vae.alpha * kl_loss

#!^L Combine loss with annealing
# Compute loss
        diff_loss= self.criterion(eps, eps_pred)
        loss = annealing_weight()*vae_loss + diff_loss

#!^L Scaling loss -> Backprop
        optim.zero_grad()

        loss_scaler.scale(loss).backward()
        loss_scaler.unscale_(optim)   
...
#!^L Upldate the scaled value
        ...
        self.log("diffusion_loss", diff_loss, prog_bar=True)
        self.log("vae_loss", vae_loss, prog_bar=True)
        self.log("annealing_weight", annealing_weight(), prog_bar=True) # Adjust
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            list(self.online_network.decoder.parameters())+list(self.vae.parameters()) , lr=self.lr # vae 
        )
        #^L For using different learning rate for different weight
        # optim.SGD([
