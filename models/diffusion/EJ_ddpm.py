...
class DDPM(nn.Module):
    ...
    def get_posterior_mean_covariance(
        self, x_t, t, clip_denoised=True, cond=None, z_vae=None, guidance_weight=0.0, adaptive_weight=None
    ):
        B = x_t.size(0)
        ...
        #################################
        effective_guidance = guidance_weight
        if adaptive_weight is not None:
            effective_guidance = guidance_weight * adaptive_weight
        ###############################

        # Compute updated score
        if effective_guidance == 0:
            eps_score = self.decoder(x_t, t_, low_res=cond, z=z_vae)
        else:
            eps_score = (1 + effective_guidance) * self.decoder(
                x_t, t_, low_res=cond, z=z_vae
            ) - effective_guidance * self.decoder(
                x_t,
                t_,
    ...
    def sample(
        ...
        for idx, t in enumerate(reversed(range(0, num_steps))):
            ...
            (
                ...
                x, t, cond=cond, z_vae=z_vae, guidance_weight=guidance_weight, adaptive_weight=adaptive_weight #
            )
            nonzero_mask = (
    ...
    # def forward(self, x, eps, t, low_res=None, z=None, adaptive_weight=None): #
    #     # Predict noise
    ...
    def forward(self, x, eps, t, low_res=None, z=None, adaptive_weight=None):
        ...
        if adaptive_weight is not None:
            conditional_pred = self.decoder(x_t, t, low_res=low_res, z=z)
            unconditional_pred = self.decoder(
                x_t,
                t,
                low_res=torch.zeros_like(low_res) if low_res is not None else None,
                z=torch.zeros_like(z) if z is not None else None
            )
            return conditional_pred + adaptive_weight * (conditional_pred - unconditional_pred)
        return self.decoder(x_t, t, low_res=low_res, z=z)